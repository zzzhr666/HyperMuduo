...
HyperMuduo 提供了内置的基准测试工具，用于评估其在极限负载下的吞吐量和延迟。

### 测试场景：Echo 回显测试
该测试通过模拟 N 个并发连接，向服务器发送固定大小的数据包并接收回显，以此计算 QPS 和吞吐量。

### 1. 编译 Benchmark
构建项目时会自动编译 `echo_server` 和 `echo_client`。

### 2. 运行测试

首先，启动服务器：
```bash
./build/benchmark/echo_server 8888
```

然后，在另一个终端启动客户端（默认 100 连接，1KB 数据包，持续 10 秒）：
```bash
./build/benchmark/echo_client 127.0.0.1 8888
```

### 3. 测试结果 (参考数据)

以下是在本地环境下的测试结果。为了压榨库的性能极限，服务端开启了多线程 Reactor 模式，客户端采用了多线程并发模型。
...
**测试条件**：
- **服务端**: 16 线程 (EventLoopThreadPool)，关闭所有日志打印
...
Total Throughput : 834.32 MB/s
Total QPS        : 854346.40 msgs/s
Total Messages   : 8543464
Total Bytes      : 8748507136
============================================
```

*   **Throughput (吞吐量)**：服务器每秒处理的数据量，达到了约 **834 MB/s**。
*   **QPS**：每秒处理的回显次数，达到了约 **85 万**。

### 4. 测试结果说明与局限性 (Honest Report)

为了确保数据的透明与真实，以下几点必须说明：
*   **场景理想化 (Echo Only)**：该测试仅涉及底层的 `read` 和 `send` 操作，不包含任何业务逻辑（如序列化、数据库查询、复杂计算等）。在实际复杂的业务场景下（例如引入 Protobuf 编解码），QPS 通常会下降至 **10 万 - 30 万** 左右。
*   **WSL2 环境开销**：测试运行在 WSL2 (Hyper-V) 虚拟化环境中，网络包需经过虚拟网桥，存在一定损耗。如果在 **原生 Linux 物理机** 上运行，消除了虚拟化开销，QPS 极有可能 **突破 100 万**。
*   **小包压力测试**：该测试使用的是 1KB 大小的消息，主要验证高频小包下的系统调用和内存拷贝性能。如果是大文件流式传输，受限于带宽和 IO 策略，吞吐量数值会有所不同。
